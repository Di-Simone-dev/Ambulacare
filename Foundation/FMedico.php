    ...
    */
    ...
    public static function loadByField($field, $id)
    {
        ...
        if (($result != null) && ($rows_number == 1)) {
            ...
            $cli = new EMedico($ute->getNome(), $ute->getCognome(), $ute->getEmail(), $ute->getPassword(),$ute->getAttivo(),$ute->getCosto(),$ute->getTipologia());
        } else {
            if (($result != null) && ($rows_number > 1)) {
                $tra = array();
                for ($i = 0; $i < count($result); $i++) {
                    $ute[] = FUtenteloggato::loadByField("email", $result[$i]["email"]);
                    $cli[] = new EMedico($ute[$i]->getNome(), $ute[$i]->getCognome(), $ute[$i]->getEmail(), $ute[$i]->getPassword(),$ute->getAttivo(),$ute->getCosto(),$ute->getTipologia());
                }
            }
        }
        return $cli;
